...
router.get('/', async (req, res) => {
  const { rows } = await pool.query('SELECT * FROM facturas_ventas');
  res.json(rows);
});

// Registrar nueva venta, EL TRIGGER VA A CARGAR LAS CUENTAS A COBRAR
router.post('/', async (req, res) => {
  const {
    cliente_id, fecha, total, tipo,
  ...
  const result = await pool.query(
    `INSERT INTO facturas_ventas (
     ...
     RETURNING *`,
    [nro_factura, cliente_id, fecha, total, tipo, cuotas, modalidad, dias_cuotas, talonario_id]
  );
